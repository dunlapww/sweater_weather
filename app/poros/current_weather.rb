class CurrentWeather
  ...
  def initialize(weather_data)
    ...
    @daily_weather = daily_weather(weather_data[:daily], 5)
    @hourly_weather = hourly_weather(weather_data[:hourly], 8)
  end

  def daily_weather(weather_days, days)
    weather_days.first(days).map do |day|
      WeatherDay.new(day)
    end
  end

  def hourly_weather(weather_hours, hours)
