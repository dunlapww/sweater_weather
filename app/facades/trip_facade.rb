class TripFacade
  def self.get_trip(trip_params)
    ...
    trip_data = collect_trip_data(trip_params, trip_response, dest_weather)
    Trip.new(trip_data)
  end
  ...
  def self.collect_trip_data(trip_params, trip_response, dest_weather)
    travel_time = travel_secs(trip_response[:route][:legs][0][:maneuvers])
    arrival_time = Time.zone.now + travel_time
